...
import Metadata, { Slot } from "../typings/metadata.ts";
...
export const selectIsRehydrating = createSelector(
  selector,
  (state) => state.isRehydrating,
);
export const selectIsDeleting = (name: string) => (state: RootState) =>
  state.slot.isDeleting[name];
