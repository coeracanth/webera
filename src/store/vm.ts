import { VM } from "../../deps/erajs.ts";
import jszip from "../../deps/jszip.ts";
import localforage from "../../deps/localforage.ts";
import pako from "../../deps/pako.ts";
import { createSelector } from "../../deps/reselect.ts";
import { createAction, createReducer } from "../../deps/typesafe-actions.ts";

import * as era from "../era.ts";
import Metadata from "../typings/metadata.ts";
import { createSubReducer, State as RootState, ThunkAction } from "./index.ts";
import { clearBlocks, popBlock, pushBlock } from "./log.ts";
...
const selectInput = createSelector(
  selector,
  (state) => state.inputs[0] as Input | undefined,
);
  ...
  return async (dispatch, getState) => {
    ...
    const iterator = vm.start({
      getSavedata: async (key) => {
        const bytes = await localforage.getItem<Uint8Array>(
