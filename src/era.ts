...
export async function extract(zip: JSZip): Promise<Map<string, string>> {
  ...
  const erbDir = Object.values(zip.files).find((zipObj) =>
    zipObj.dir && (
  ...
  await Promise.all(
    Object.values(zip.files).map(async (zipObj) => {
      if (
        !zipObj.name.startsWith(csvDir.name) &&
        !zipObj.name.startsWith(erbDir.name)
      ) {
        return;
      }
